...
import { MINIMUM_LIQUIDITY_THRESHOLD_ETH, ONE_BD } from "./helpers/constants";
...
export function findEthPerToken(token: Token): BigDecimal {
  ...
  for (let i = 0; i < whitelist.length; ++i) {
    ...
    if (!pair) {
      log.error("Unknown pair: {}", [pairAddress]);
      continue;
    }
  ...
  return BigDecimal.zero(); // nothing was found return 0
}
