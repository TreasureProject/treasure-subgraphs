...
import { MAGICSWAP_TOKEN_WHITELIST } from "@treasure/constants";
...
export function getTrackedVolumeUSD(
  ...
  if (pair.liquidityProviderCount.lt(BigInt.fromI32(5))) {
    ...
    if (
      MAGICSWAP_TOKEN_WHITELIST.includes(token0.id) &&
      MAGICSWAP_TOKEN_WHITELIST.includes(token1.id)
    ) {
...
function isCompleteMint(mintId: string): boolean {
  ...
  return !mint || !mint.sender; // sufficient checks
}
...
export function onTransfer(event: TransferEvent): void {
  ...
  if (event.params.from == ADDRESS_ZERO) {
    ...
    if (
      ...
      isCompleteMint(mints[mints.length - 1])
    ) {
    ...
    const burn = new Burn(
      event.transaction.hash
        .toHex()
        .concat("-")
        .concat(BigInt.fromI32(transaction.burns.length).toString())
    );
    burn.pair = pair.id;
  ...
  if (
    ...
    const fromUserLiquidityPosition = createLiquidityPosition(
      event.params.from,
      event.address,
      event.block
    );
    ...
    fromUserLiquidityPosition.liquidityTokenBalance =
      fromUserLiquidityPosition.liquidityTokenBalance.minus(value);
  ...
  if (event.params.to != ADDRESS_ZERO && event.params.to.toHex() != pair.id) {
    const toUserLiquidityPosition = createLiquidityPosition(
      event.params.to,
      event.address,
      event.block
    );
    ...
    toUserLiquidityPosition.liquidityTokenBalance =
      toUserLiquidityPosition.liquidityTokenBalance.plus(value);
...
export function onSync(event: SyncEvent): void {
  ...
  if (!pair) {
    log.error("Unknown pair: {}", [event.address.toHex()]);
    return;
  }
  ...
  factory.liquidityETH = factory.liquidityETH.minus(
  ...
  if (bundle.ethPrice.notEqual(BigDecimal.zero())) {
    ...
    trackedLiquidityETH = BigDecimal.zero();
  }
...
export function onMint(event: MintEvent): void {
  ...
  if (!transaction) {
    log.error("Unknown transaction: {}", [transactionId]);
    return;
  }
...
export function onBurn(event: BurnEvent): void {
  ...
  if (burn === null || burn.complete) {
    ...
    burn.complete = true;
    burn.pair = pair.id;
    burn.liquidity = BigDecimal.zero();
    burn.transaction = transaction.id;
    burn.timestamp = transaction.timestamp;
  }
...
export function onSwap(event: SwapEvent): void {
  ...
  if (bundle.ethPrice.equals(BigDecimal.zero())) {
    trackedAmountETH = BigDecimal.zero();
  } else {
  ...
  if (
    !BLACKLIST_EXCHANGE_VOLUME.includes(token0.id) &&
    !BLACKLIST_EXCHANGE_VOLUME.includes(token1.id)
  ) {
  ...
  swap.amountUSD =
    trackedAmountUSD == BigDecimal.zero() ? derivedAmountUSD : trackedAmountUSD;
  swap.save();
  ...
  token0DayData.volumeETH = token0DayData.volumeETH.plus(
  ...
  token1DayData.volumeETH = token1DayData.volumeETH.plus(
