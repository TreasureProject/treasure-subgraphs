...
export function updateMetrics(presale: MemePresale, event: ethereum.Event): void {
...
export function updateTransactionArrays(account: Account, presaleId: string, type: string): void {
  if (type == 'BUY') {
    let buyTransactions = account.buyTransactions;
    if (!buyTransactions.includes(presaleId)) {
      buyTransactions.push(presaleId);
